...
class Data extends CI_Controller
{
	function __construct()
	{
		...
		$this->load->helper(array('form', 'url', 'upload', 'perpus'));
	...
	public function bukudetail()
	{
		...
		if ($count > 0) {
			...
			$this->data['kats'] = $this->db->where('deleted_at IS NULL', NULL, FALSE)->order_by('id_kategori', 'DESC')->get('tbl_kategori')->result_array();
			$this->data['rakbuku'] = $this->db->where('deleted_at IS NULL', NULL, FALSE)->order_by('id_rak', 'DESC')->get('tbl_rak')->result_array();
	...
	public function prosesbuku()
	{
		...
		if (!empty($this->input->post('tambah'))) {
			...
			if (!empty($_FILES['gambar']['name'])) {
				...
				$config = perpus_upload_config('assets/image/buku', 'gif|jpg|jpeg|png', 4096);
				// load library upload
			...
			if (!empty($_FILES['lampiran']['name'])) {
				// setting konfigurasi upload
				$config = perpus_upload_config('assets/image/buku', 'pdf', 10240);
				// load library upload
				$this->load->library('upload', $config);
				$this->upload->initialize($config);
		...
		if (!empty($this->input->post('edit'))) {
			...
			if (!empty($_FILES['gambar']['name'])) {
				...
				if ($this->upload->do_upload('gambar')) {
					$this->upload->data();
					$gambar = FCPATH . 'assets/image/buku/' . basename((string) $post['gmbr']);
			...
			if (!empty($_FILES['lampiran']['name'])) {
				...
				if ($this->upload->do_upload('lampiran')) {
					$this->upload->data();
					$lampiran = FCPATH . 'assets/image/buku/' . basename((string) $post['lamp']);
	...
	public function kategori()
	{
		...
		$this->data['idbo'] = $this->session->userdata('ses_id');
		$this->data['kategori'] = $this->db->where('deleted_at IS NULL', NULL, FALSE)->order_by('id_kategori', 'DESC')->get('tbl_kategori');

		if (!empty($this->input->get('id'))) {
			$id = $this->input->get('id');
			$count = $this->M_Admin->CountTableId('tbl_kategori', 'id_kategori', $id);
			if ($count > 0) {
				$this->data['kat'] = $this->db->where('id_kategori', $id)->where('deleted_at IS NULL', NULL, FALSE)->get('tbl_kategori')->row();
			} else {
	...
	public function katproses()
	{
		...
		if (!empty($this->input->get('kat_id'))) {
			$this->M_Admin->delete_table('tbl_kategori', 'id_kategori', $this->input->get('kat_id'));
	...
	public function rak()
	{
		...
		$this->data['idbo'] = $this->session->userdata('ses_id');
		$this->data['rakbuku'] = $this->db->where('deleted_at IS NULL', NULL, FALSE)->order_by('id_rak', 'DESC')->get('tbl_rak');

		if (!empty($this->input->get('id'))) {
			$id = $this->input->get('id');
			$count = $this->M_Admin->CountTableId('tbl_rak', 'id_rak', $id);
			if ($count > 0) {
				$this->data['rak'] = $this->db->where('id_rak', $id)->where('deleted_at IS NULL', NULL, FALSE)->get('tbl_rak')->row();
			} else {
	...
	public function rakproses()
	{
		...
		if (!empty($this->input->get('rak_id'))) {
			$this->M_Admin->delete_table('tbl_rak', 'id_rak', $this->input->get('rak_id'));
